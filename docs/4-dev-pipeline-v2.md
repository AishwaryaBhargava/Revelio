...
**Project Development Pipeline — V2.0**
...
### Goal:
Fix the suggestion card click behavior so the chat shows a clean, readable user message instead of the raw title and preview text concatenated together. The response quality stays the same -- only the display improves.

### What we do:

**Frontend: Suggestion card click message**
- [ ] Change `handleCardClick` in `Landing.tsx` to send just the card title as the user-facing message
- [ ] Pass the full card context (type, title, preview) as a separate field to the chat endpoint for backend context
- [ ] Update `ChatRequest` schema in backend to accept an optional `card_context` field
- [ ] Update `build_chat_prompt` in `prompt_builder.py` to include card context in the system prompt when present
- [ ] Ensure the user bubble in chat shows only the clean title, not the full preview

**Frontend: Chat message display**
- [ ] Add a subtle card type label above the user message when a suggestion was clicked (e.g. a small ANSWER or QUESTION TO ASK chip)
- [ ] Keep free-typed messages looking exactly as they do now
- [ ] Ensure timestamps are correct on both message types

**Backend: prompt_builder.py**
- [ ] When `card_context` is present, add it as additional context to the system prompt: "The user clicked a suggestion card of type [type] with preview: [preview]. Use this as additional context for your answer."
- [ ] When `card_context` is absent, use the existing chat prompt unchanged

### Acceptance Criteria:
- Clicking a suggestion card sends only the card title as the visible user message in chat
- The chat response is still grounded in both the transcript and the card context
- Free-typed messages are unaffected
- The user bubble shows a clean short message, not a long concatenated string
- Optional card type chip is visible above suggestion-triggered messages

### Outcome:
A cleaner, more readable chat experience where suggestion clicks feel natural and intentional rather than pasting raw card text into the input.

### Notes / References:
- The card preview should still be passed to the backend as context -- just not shown to the user as the message
- Do not change the chat prompt for free-typed messages

### Testing:
- Click 3 different suggestion types, confirm each shows only the card title in the user bubble
- Confirm the assistant response still references the transcript and card context
- Type a free message, confirm it is unaffected
- Check timestamps on both message types are correct

### Evaluation:
- User bubble for suggestion clicks is under 15 words
- Response quality is equal to or better than before the change
- Zero visual regressions on free-typed chat messages
...
### Goal:
Redesign the 3-column desktop layout into a responsive tab-based layout for mobile and tablet screens, while keeping the desktop experience exactly as it is. Users on small screens can switch between Transcript, Suggestions, and Chat using a bottom navigation tab bar.

### What we do:

**Layout: Responsive breakpoints**
- [ ] Define breakpoint: desktop is 1024px and above (3-column layout unchanged), mobile/tablet is below 1024px (tab-based layout)
- [ ] On mobile, show only one panel at a time based on the active tab
- [ ] On desktop, show all 3 columns side by side as before
- [ ] Use CSS media queries or a `useWindowSize` hook to detect the current breakpoint

**Mobile: Bottom tab navigation**
- [ ] Create a `BottomNav.tsx` component with 3 tabs: Transcript, Suggestions, Chat
- [ ] Each tab has an icon (Mic, Lightbulb, MessageSquare from lucide-react) and a label
- [ ] Active tab is highlighted with the teal accent color
- [ ] Tab bar is fixed to the bottom of the screen on mobile only
- [ ] Show a badge on the Suggestions tab when new suggestions arrive
- [ ] Show a badge on the Chat tab when a new message arrives

**Mobile: Panel layout**
- [ ] Each panel takes full screen height minus the header and tab bar
- [ ] Panels scroll independently within their container
- [ ] MicButton and transcript feed stack vertically with comfortable spacing
- [ ] Suggestion cards are full width on mobile
- [ ] Chat input stays pinned to the bottom above the tab bar

**Mobile: Header**
- [ ] Keep the Revelio logo and wordmark
- [ ] Move Export and Settings buttons into a compact icon-only format on mobile
- [ ] Header height reduces slightly on mobile

**Tablet: Layout**
- [ ] On tablet (768px to 1023px), use the same tab-based layout as mobile
- [ ] Slightly larger card sizes and font sizes than mobile

**Testing across devices**
- [ ] Test on iPhone SE (375px), iPhone 14 (390px), iPad (768px), and desktop (1280px+)
- [ ] Confirm no horizontal scroll on any viewport
- [ ] Confirm all interactive elements have comfortable tap targets (min 44px)

### Acceptance Criteria:
- On screens below 1024px, a bottom tab bar is visible with 3 tabs
- Switching tabs shows the correct panel and hides the others
- The desktop 3-column layout is completely unchanged at 1280px+
- No horizontal overflow on any screen size
- All tap targets are at least 44px tall on mobile
- Badge appears on Suggestions tab when new batch arrives
- Badge appears on Chat tab when new assistant message arrives
- Export and Settings are accessible on mobile via header icons
- Chat input stays above the tab bar and does not get covered by the keyboard

### Outcome:
A fully responsive Revelio that works on any device. Desktop users get the full 3-column experience. Mobile and tablet users get a clean tab-based layout with the same core functionality.

### Notes / References:
- Do not use a library for the tab navigation -- keep it simple with useState and CSS
- The recording indicator (Live dot) should be visible regardless of which tab is active on mobile
- Test keyboard behavior on mobile carefully -- the virtual keyboard pushing the chat input up is a common issue on iOS
- Consider adding a small "recording" banner at the top of suggestions and chat panels on mobile so users know the mic is active even when not on the transcript tab

### Testing:
- Open on a real iPhone and iPad if possible, not just Chrome DevTools
- Switch between all 3 tabs while recording, confirm transcript and suggestions update in the background
- Click a suggestion on mobile, confirm it switches to the Chat tab and sends the message
- Open Settings modal on mobile, confirm it is scrollable and all fields are accessible
- Export on mobile, confirm the JSON downloads correctly
- Test in both portrait and landscape orientations

### Evaluation:
- App is fully usable on a real mobile device without any horizontal scrolling or cut-off content
- Tab switching feels instant (under 100ms)
- Suggestion badge correctly reflects new batch arrivals
- Desktop layout pixel-perfect match to pre-Phase 8 state
- All acceptance criteria pass on Chrome mobile and Safari mobile
